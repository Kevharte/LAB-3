...
public class HandPoker extends Hand {
	...
	public boolean isFullHouse() {
		...
		if(this.getCRC().get(0).getiCnt() == Constants.THREE_OF_A_KIND && this.getCRC().get(3).getiCnt() == Constants.TWO_OF_A_KIND) {
			HandScorePoker HSP = (HandScorePoker) this.getHS();
			HSP.seteHandStrength(eHandStrength.FullHouse);
			int iGetCard = this.getCRC().get(0).getiCardPosition();
			HSP.setHiCard(this.getCards().get(iGetCard));
			HSP.setLoCard(this.getCards().get(this.getCRC().get(3).getiCardPosition()));
			HSP.setKickers(FindTheKickers(this.getCRC()));
			this.setHS(HSP);
			bisFullHouse = true;
		}
	...
	public boolean isThreeOfAKind() {
		...
		if (this.getCRC().size() == 3 && this.getCRC().get(3).geteRank() != this.getCRC().get(4).geteRank()) {
